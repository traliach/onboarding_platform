...
| 2026-04-24 | `ansible-playbook playbooks/site.yml ...` after temp-path fix | The `common` role reached the first `node_exporter` task, then failed because `node_exporter_version` was reported undefined even though it exists in `group_vars/all/main.yml`. The real issue was repo layout: `playbooks/`, `inventory/`, and `group_vars/` are siblings, so Ansible did not auto-discover `group_vars` from this path. | Load `../group_vars/all/main.yml` and `../group_vars/all/vault.yml` explicitly in `playbooks/site.yml`, then rerun the playbook. |
